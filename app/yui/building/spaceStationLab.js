  ...
  (function () {
    ...
    Lang.extend(SpaceStationLab, Lacuna.buildings.Building, {
      ...
      _getPlanTab: function () {
        ...
        Event.delegate(
          ...
          function (e, matchedEl, container) {
            ...
            Dom.get('stationLabPlanSelected').innerHTML = Sel.query(
              'div.buildingName',
        ...
        return new YAHOO.widget.Tab({
          ...
          content: [
            ...
            '                <th><img src="',
            Lib.AssetUrl,
            'ui/s/time.png" title="Time" class="smallTime" /></th>',
            '                <th></th>',
            '            </tr>',
            '        </table>',
            ...
            '    <button type="button" id="stationLabMakingSubsidize">Subsidize for ',
            this.result.make_plan.subsidy_cost,
            ' <img src="',
            Lib.AssetUrl,
            'ui/s/essentia.png" class="smallEssentia" style="vertical-align: middle;" /></button>',
            '</div>',
          ].join(''),
        });
      },
      buildPlans: function (types) {
        ...
        for (var n = 0; n < types.length; n++) {
          ...
          frag[frag.length] = [
            ...
            Lib.AssetUrl,
            'planet_side/surface-station.jpg) no-repeat center;text-align:center">',
            '            <img src="',
            Lib.AssetUrl,
            'planet_side/',
            plan.image,
      ...
      buildLevels: function (levelCosts) {
        var frag = [
            '<table id="stationLabLevels" class="buildingStats" cellpadding="0" cellspacing="0"><col width="53" /><colgroup span="6" width="110" />',
          ],
          planet = BodyRPCStore;
      ...
      PlanPopulate: function () {
        ...
        if (makePlan.making) {
          ...
          this.addQueue(
            this.result.building.work.seconds_remaining,
            this.makingQueue,
            Dom.get('stationLabMakingTime')
          );
        } else {
          Dom.setStyle('stationLabPlansContainer', 'display', '');
          Dom.setStyle('stationLabLevelsContainer', 'display', 'none');
