...
if (
  typeof YAHOO.lacuna.buildings.BlackHoleGenerator == 'undefined' ||
    ...
    Lang.extend(BlackHoleGenerator, Lacuna.buildings.Building, {
      ...
      _getBHGTab: function () {
        ...
        this.tab = new YAHOO.widget.Tab({
          ...
          content: [
            ...
            '  <span id="bhgTargetSelectOrbit" style="display:none;">Orbit: <select id="bhgTargetOrbit"><option value="">None (target the star)</option>',
            orbits,
            '</select></span>',
            ...
            '    <li>You may subsidize the cool-down for 2 <img src="',
            Lib.AssetUrl,
            'ui/s/essentia.png" class="smallEssentia" />.</li>',
        ...
        if (actions.length === 0) {
          ...
          for (var i = 0; i < actions.length; i++) {
            ...
            if (task.name === 'Change Type') {
              ...
              typeSelector =
                '<select id="bhgChangeTypeSelect"><option value="">New ' + label + ' Type</option>';

              var options =
                task.body_type == 'asteroid'
                  ? this.result.task_options.asteroid_types
                  : this.result.task_options.planet_types;

              for (var j = 0; j < options.length; j++) {
                typeSelector =
                  typeSelector +
                  ['<option value="', options[j], '">', options[j], '</option>'].join('');
              }
            ...
            nLi.innerHTML = [
              ...
              waste_out,
              '      Recovery Time: ',
              Lib.formatTime(task.recovery),
              '    </div>',
              '  </div>',
              '  <div class="yui-u" style="width:25%; text-align:right;">',
              canGenerate === 1
                ? typeSelector +
                  '<button type="button" name="generate">Generate</button><button type="button" name="subsidize">Subsidize</button>'
                : '<b>',
              task.reason,
              '</b>',
              '  </div>',
            ...
            if (task.success > 0) {
              Event.on(
                Sel.query('button[name=generate]', nLi, true),
                'click',
                this.bhgGenerate,
                {
                  Self: this,
              ...
              Event.on(
                Sel.query('button[name=subsidize]', nLi, true),
                'click',
                this.bhgGenerate,
                {
                  Self: this,
      ...
      bhgParseResult: function (result, type) {
        ...
        if (result.message === 'Swapped Places') {
          out =
            out + [result.message, ' with ', result.swapname, ' at orbit ', result.orbit].join('');
        } else if (result.message === 'Changed Size') {
          out =
            out +
            [result.name, ' changed size from ', result.old_size, ' to ', result.size].join('');
          ...
          out =
            out +
            [result.name, ' is now a type ', newtype, ' asteroid of size ', result.size].join('');
        } else if (result.message === 'Made Planet') {
          var newtype = result['class'].replace(new RegExp('.*::', 'g'), '');
          out =
            out +
            [result.name, ' is now a type ', newtype, ' planet of size ', result.size].join('');
        } else {
          out = out + [result.message, ' at ', result.name].join('');
        }
