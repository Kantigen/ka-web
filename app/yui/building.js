...
if (typeof YAHOO.lacuna.buildings.Building == 'undefined' || !YAHOO.lacuna.buildings.Building) {
    ...
    Building.prototype = {
      ...
      getTabs: function () {
        if (this.building.efficiency * 1 < 100 && this.building.repair_costs) {
          ...
          if (
            this.building.upgrade.production &&
            this.building.food_capacity * 1 +
              this.building.ore_capacity * 1 +
              this.building.water_capacity * 1 +
              this.building.energy_capacity * 1 +
              this.building.waste_capacity * 1 >
              0
          ) {
      ...
      _details: function (info) {
        ...
        return [
          '<li><span class="smallImg"><img src ="',
          Lib.AssetUrl,
          'ui/s/',
          info.type,
          '.png" title="',
          ...
          numclass,
          '" title="',
          Lib.formatNumber(info.amount),
          '">',
          util.reduceNumber(info.amount),
          info.suffix,
          '</span></li>',
        ].join('');
      },
      ...
      _getRepairTab: function () {
        this.repairTab = new YAHOO.widget.Tab({
          ...
          content: [
            ...
            '    <span id="repairText">Building is currently running at ',
            this.building.efficiency,
            '% efficiency.  Costs to repair the building are:</span>',
            '    <ul>',
            this._details({
              type: 'food',
              amount: this.building.repair_costs.food,
            }),
            this._details({
              type: 'ore',
              amount: this.building.repair_costs.ore,
            }),
            this._details({
              type: 'water',
              amount: this.building.repair_costs.water,
            }),
            this._details({
              type: 'energy',
              amount: this.building.repair_costs.energy,
            }),
            '    </ul>',
      ...
      _getProductionTab: function () {
        ...
        this.productionTab = new YAHOO.widget.Tab({
          ...
          content: [
            ...
            this._details({
              type: 'food',
              amount: this.building.food_hour,
              suffix: '/hr',
            }),
            this._details({
              type: 'ore',
              amount: this.building.ore_hour,
              suffix: '/hr',
            }),
            this._details({
              type: 'water',
              amount: this.building.water_hour,
              suffix: '/hr',
            }),
            this._details({
              type: 'energy',
              amount: this.building.energy_hour,
              suffix: '/hr',
            }),
            this._details({
              type: 'waste',
              amount: this.building.waste_hour,
              suffix: '/hr',
            }),
            this._details({
              type: 'happiness',
              amount: this.building.happiness_hour,
              suffix: '/hr',
            }),
            ...
            up
              ? [
                  ...
                  this._details({
                    ...
                    suffix: '/hr',
                    numclass:
                      this.building.food_hour - up.production.food_hour > planet.food_hour
                        ? 'low-resource'
                        : null,
                  }),
                  this._details({
                    ...
                    suffix: '/hr',
                    numclass:
                      this.building.ore_hour - up.production.ore_hour > planet.ore_hour
                        ? 'low-resource'
                        : null,
                  }),
                  this._details({
                    ...
                    suffix: '/hr',
                    numclass:
                      this.building.water_hour - up.production.water_hour > planet.water_hour
                        ? 'low-resource'
                        : null,
                  }),
                  this._details({
                    ...
                    suffix: '/hr',
                    numclass:
                      this.building.energy_hour - up.production.energy_hour > planet.energy_hour
                        ? 'low-resource'
                        : null,
                  }),
                  this._details({
                    type: 'waste',
                    amount: up.production.waste_hour,
                    suffix: '/hr',
                  }),
                  this._details({
                    type: 'happiness',
                    amount: up.production.happiness_hour,
                    suffix: '/hr',
                  }),
                  up.can
                    ? '<li><button id="buildingDetailsUpgrade" type="button">Upgrade to Level ' +
                      (1 + this.building.level * 1) +
                      '</button></li>'
                    : '<li class="alert">Unable to Upgrade:</li><li class="alert">',
                  up.reason[1],
                  '</li>',
                ].join('')
              : '',
            ...
            up
              ? [
                  ...
                  this._details({
                    ...
                    numclass: up.cost.food > planet.food_stored ? 'low-resource' : null,
                  }),
                  this._details({
                    type: 'ore',
                    amount: up.cost.ore || 0,
                    numclass: up.cost.ore > planet.ore_stored ? 'low-resource' : null,
                  }),
                  this._details({
                    type: 'water',
                    amount: up.cost.water || 0,
                    numclass: up.cost.water > planet.water_stored ? 'low-resource' : null,
                  }),
                  this._details({
                    type: 'energy',
                    amount: up.cost.energy || 0,
                    numclass: up.cost.energy > planet.energy_stored ? 'low-resource' : null,
                  }),
                  this._details({
                    type: 'waste',
                    amount: up.cost.waste || 0,
                  }),
                  '    <li><span class="smallImg"><img src="',
                  Lib.AssetUrl,
                  'ui/s/time.png" title="Time" class="smallTime" /></span><span class="buildingDetailsNum">',
                  Lib.formatTime(up.cost.time),
                  '</span></li>',
                ].join('')
              : '',
            currentLevel <= 1
              ? ''
              : down.can
              ? '<li><button id="buildingDetailsDowngrade" type="button">Downgrade to Level ' +
                (currentLevel - 1) +
                '</button></li>'
              : '<li class="alert">Unable to Downgrade:</li><li class="alert">' +
                String(down.reason).replace(/^\d+,\s*/, '') +
                '</li>',
            '            </ul>',
            '        </div>',
            '    </div>',
      ...
      Demolish: function () {
        ...
        if (
          confirm(
            [
              'Are you sure you want to Demolish the level ',
              building.level,
              ' ',
              building.name,
              '?',
      ...
      Downgrade: function () {
        ...
        if (
          confirm(
            [
              'Are you sure you want to downgrade the level ',
              building.level,
              ' ',
              building.name,
              '?',
      ...
      _getIncomingSupplyChainsTab: function () {
        ...
        this.incomingSupplyChainTab.subscribe(
          'activeChange',
          this.viewIncomingSupplyChainInfo,
          this,
          true
        );
      ...
      incomingSupplyChainList: function () {
        ...
        for (var i = 0; i < supply_chains.length; i++) {
          ...
          nLi.innerHTML = util.reduceNumber(chain.resource_hour);
      ...
      _getStorageTab: function () {
        var p = this.building.upgrade.production,
          output = [
            ...
            this._details({
              type: 'food',
              amount: this.building.food_capacity,
            }),
            this._details({
              type: 'ore',
              amount: this.building.ore_capacity,
            }),
            this._details({
              type: 'water',
              amount: this.building.water_capacity,
            }),
            this._details({
              type: 'energy',
              amount: this.building.energy_capacity,
            }),
            this._details({
              type: 'waste',
              amount: this.building.waste_capacity,
            }),
            '        </ul>',
            '    </div>',
            '    <div class="yui-u">',
            ...
            this._details({
              type: 'food',
              amount: p.food_capacity,
            }),
            this._details({ type: 'ore', amount: p.ore_capacity }),
            this._details({
              type: 'water',
              amount: p.water_capacity,
            }),
            this._details({
              type: 'energy',
              amount: p.energy_capacity,
            }),
            this._details({
              type: 'waste',
              amount: p.waste_capacity,
            }),
            '        </ul>',
            '    </div>',
            '</div>',
  ...
  YAHOO.register('building', YAHOO.lacuna.buildings.Building, {
    version: '1',
    build: '0',
  });
}
