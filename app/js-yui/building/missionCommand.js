...
if (
    typeof YAHOO.lacuna.buildings.MissionCommand == 'undefined' ||
    !YAHOO.lacuna.buildings.MissionCommand
) {
    (function() {
        var Lang = YAHOO.lang,
            Util = YAHOO.util,
                    ...
                    if (missions && missions.length > 0) {
                        for (var i = 0; i < missions.length; i++) {
                            ...
                            nLi.innerHTML = [
                                ...
                                ms.name,
                                '</div>',
                                '        <span style="float:right">Mission ID: ',
                                ms.id,
                                '</span>',
                                '        <div class="missionPosted">Posted: ',
                                Lib.formatServerDate(ms.date_posted),
                                '</div>',
                                '        <div class="missionUniversity">Max University: ',
                                ms.max_university_level,
                                '</div>',
                                '    </div>',
                                '    <div class="yui-u">',
                                '        <button type="button" id="complete',
                                ms.id,
                                '" class="missionComplete">Complete</button>',
                                '        <button type="button" id="skip',
                                ms.id,
                                '" class="missionSkip">Skip</button>',
                                '    </div>',
                                '</div>',
                            ...
                            Event.on(
                                Sel.query('button.missionComplete', nLi, true),
                                'click',
                                this.completeMission,
                                { Self: this, Mission: ms, Line: nLi },
                                true
                            );
                            Event.on(
                                Sel.query('button.missionSkip', nLi, true),
                                'click',
                                this.skipMission,
                                { Self: this, Mission: ms, Line: nLi },
                                true
