    ...
    (function() {
        ...
        Lang.extend(Shipyard, Lacuna.buildings.Building, {
            ...
            _getQueueTab: function() {
                ...
                div.innerHTML = [
                    '<div>You may subsidize the build queue for 1 <img src="',
                    Lib.AssetUrl,
                    'ui/s/essentia.png" class="smallEssentia" /> per ship. <button type="button" class="shipQueueSubsidize">Subsidize</button> </div>',
                ...
                Event.on(
                    Sel.query('.shipQueueSubsidize', div, true),
                    'click',
                    this.SubsidizeBuildQueue,
                    this,
                    true
                );
            ...
            SetDocksAvailableMessage: function() {
                ...
                if (sda) {
                    if (this.ships.docks_available) {
                        ...
                        sda.innerHTML =
                            'You have no docks available.  Do you still have a Space Port?';
                    }
                }
            },
            ShipPopulate: function() {
                ...
                if (details) {
                    var ships = this.ships.buildable,
                        ...
                        defReason = !this.ships.docks_available
                            ? 'No docks available at Space Port.'
                            : undefined;
                    ...
                    for (var i = 0; i < shipNames.length; i++) {
                        ...
                        nLi.innerHTML = [
                            ...
                            ship.type_human,
                            '</span>: ',
                            '        <div class="shipDesc" style="display:none;">',
                            Game.GetShipDesc(shipName),
                            '</div>',
                            '        <div><label style="font-weight:bold;">Cost:</label>',
                            '            <span style="white-space:nowrap;"><img src="',
                            Lib.AssetUrl,
                            'ui/s/food.png" title="Food" class="smallFood" />',
                            Lib.formatNumber(ship.cost.food),
                            '</span>',
                            '            <span style="white-space:nowrap;"><img src="',
                            Lib.AssetUrl,
                            'ui/s/ore.png" title="Ore" class="smallOre" />',
                            Lib.formatNumber(ship.cost.ore),
                            '</span>',
                            '            <span style="white-space:nowrap;"><img src="',
                            Lib.AssetUrl,
                            'ui/s/water.png" title="Water" class="smallWater" />',
                            Lib.formatNumber(ship.cost.water),
                            '</span>',
                            '            <span style="white-space:nowrap;"><img src="',
                            Lib.AssetUrl,
                            'ui/s/energy.png" title="Energy" class="smallEnergy" />',
                            Lib.formatNumber(ship.cost.energy),
                            '</span>',
                            '            <span style="white-space:nowrap;"><img src="',
                            Lib.AssetUrl,
                            'ui/s/waste.png" title="Waste" class="smallWaste" />',
                            Lib.formatNumber(ship.cost.waste),
                            '</span>',
                            '            <span style="white-space:nowrap;"><img src="',
                            Lib.AssetUrl,
                            'ui/s/time.png" title="Time" class="smallTime" />',
                            Lib.formatTime(ship.cost.seconds),
                            '</span>',
                            '        </div>',
                            ...
                            ship.can
                                ? ' <input type="text" style="width:25px;" id="ship_' +
                                  shipName +
                                  '" value="1"> <button type="button">Build</button>'
                                : '    </div>',
                            '</div>',
                        ].join('');
                        if (ship.can) {
                            Event.on(
                                ...
                                'click',
                                this.ShipBuild,
                                { Self: this, Type: shipName, Ship: ship },
                                true
                            );
                        }
