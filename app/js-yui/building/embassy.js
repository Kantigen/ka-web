...
if (
    ...
    !YAHOO.lacuna.buildings.Embassy
) {
    (function() {
        var Lang = YAHOO.lang,
            Util = YAHOO.util,
            ...
            Lib = Lacuna.Library,
            stashSel =
                '<select><option value="1">1</option><option value="10">10</option><option value="100">100</option><option value="1000" selected="selected">1000</option><option value="10000">10000</option></select>';
        ...
        var Embassy = function(result) {
            ...
            this.isLeader =
                this.alliance && this.alliance.leader_id == Game.EmpireData.id;
        ...
        Lang.extend(Embassy, Lacuna.buildings.Building, {
            ...
            _getAllianceTab: function() {
                ...
                if (this.isLeader) {
                    div.innerHTML = [
                        '<div>',
                        '    <ul>',
                        '        <li><label>Founded: </label>',
                        Lib.formatServerDate(this.alliance.date_created),
                        '</li>',
                        '        <li><label>Alliance ID: </label>',
                        this.alliance.id,
                        '</li>',
                        '        <li><label>Description: </label><input type="text" id="embassyAllianceDesc" value="',
                        this.alliance.description,
                        '" size="50" /></li>',
                        '        <li><label>Forums: </label><input type="text" id="embassyAllianceForums" value="',
                        this.alliance.forum_uri,
                        '" size="50" /></li>',
                        '        <li><label>Announcements: </label><textarea id="embassyAllianceAnnoucements" rows="2" cols="80">',
                        this.alliance.announcements,
                        '</textarea></li>',
                    ...
                    Event.on(
                        'embassyAllianceDissolve',
                        'click',
                        this.DissolveAlliance,
                        this,
                        true
                    );
                } else {
                    div.innerHTML = [
                        ...
                        this.alliance.forum_uri
                            ...
                            : '',
                        '</li>',
                        '        <li><label>Announcements: </label>',
                        this.alliance.announcements
                            ? this.alliance.announcements.replace(
                                  '\n',
                                  '<br />'
                              )
                            : '',
            ...
            _getStashTab: function() {
                ...
                Event.delegate(
                    'embassyStashToDonate',
                    'click',
                    this.StashDonateRemove,
                    'button',
                    this,
                    true
                );
            ...
            */
                ...
                if (announce) {
                    announce.innerHTML = [
                        'Donations are unlimited. You can exchange at most ',
                        Lib.formatNumber(stash.max_exchange_size),
                        ' resources and you have ',
                        stash.exchanges_remaining_today,
                        ' exchange(s) remaining today.',
                    ].join('');
                }
            ...
            StashDonateAdd: function(e, matchedEl, container) {
                var quantity =
                        Lib.getSelectedOptionValue(matchedEl.previousSibling) *
                        1,
            ...
            StashSubmit: function() {
                ...
                if (donateTotal == 0) {
                    Dom.get('embassyStashMessage').innerHTML =
                        'Must add items to donate to Stash.';
                } else if (
                    exchangeTotal > 0 &&
                    this.stash.exchanges_remaining_today <= 0
                ) {
                    Dom.get('embassyStashMessage').innerHTML =
                        'You have already used up the amount of stash exchanges you can perform in a single day.';
                } else if (
                    exchangeTotal > 0 &&
                    donateTotal > this.stash.max_exchange_size
                ) {
                    Dom.get('embassyStashMessage').innerHTML = [
                        'You are only able to transfer ',
                        this.stash.max_exchange_size,
                        ' resources from the stash.',
                    ].join('');
                } else if (exchangeTotal > 0 && donateTotal != exchangeTotal) {
                    Dom.get('embassyStashMessage').innerHTML =
                        'Total amount of resources transfered from stash must be equal to the amount donating.';
                } else {
                    if (exchangeTotal > 0) {
                        ...
                        success: function(o) {
                            ...
                            for (n = 0; n < toDonateLis.length; n++) {
                                if (toDonateLis[n].Object) {
                                    ...
                                    toDonateLis[n].parentNode.removeChild(
                                        toDonateLis[n]
                                    );
                                }
                            }
                            for (n = 0; n < toExchangeLis.length; n++) {
                                if (toExchangeLis[n].Object) {
                                    ...
                                    toExchangeLis[n].parentNode.removeChild(
                                        toExchangeLis[n]
                                    );
                                }
                            }
                    ...
                    this.service.create_alliance(data, {
                        success: function(o) {
                            YAHOO.log(
                                o,
                                'info',
                                'Embassy.CreateAlliance.success'
                            );
                            this.rpcSuccess(o);
                            this.alliance = o.result.alliance;
                            this.isLeader =
                                this.alliance &&
                                this.alliance.leader_id == Game.EmpireData.id;
                            Dom.get('embassyCreateMessage').innerHTML = '';
            ...
            InvitesPopulate: function() {
                ...
                if (details) {
                    ...
                    for (var i = 0; i < invites.length; i++) {
                        ...
                        nLi.innerHTML =
                            'Reason:<input type="text" class="embassyActionMessage" />';
                        nUl.appendChild(nLi);
            ...
            InvitesAccept: function() {
                if (
                    confirm(
                        [
                            'Are you sure you want to accept the alliance invite from ',
                            this.Invite.name,
                            '?',
                        ].join('')
                    )
            ...
            InvitesReject: function() {
                if (
                    confirm(
                        [
                            'Are you sure you want to reject the alliance invite from ',
                            this.Invite.name,
                            '?',
                        ].join('')
                    )
                ...
                this.service.update_alliance(
                    {
                        ...
                        params: {
                            ...
                            announcements: Dom.get(
                                'embassyAllianceAnnoucements'
                            ).value,
                        },
                    },
                    {
                        success: function(o) {
                            YAHOO.log(
                                o,
                                'info',
                                'Embassy.update_alliance.success'
                            );
                            this.rpcSuccess(o);
                        ...
                        },
                        {
                            success: function(o) {
                                YAHOO.log(
                                    o,
                                    'info',
                                    'Embassy.leave_alliance.success'
                                );
                                this.rpcSuccess(o);
            ...
            DissolveAlliance: function() {
                if (
                    confirm(
                        [
                            'All Space Stations will be abandoned. Are you sure you want to dissolve ',
                            this.alliance.name,
                            '?',
                        ].join('')
                    )
                        ...
                        },
                        {
                            success: function(o) {
                                YAHOO.log(
                                    o,
                                    'info',
                                    'Embassy.dissolve_alliance.success'
                                );
                                this.rpcSuccess(o);
                                delete this.alliance;
                                this.removeTab(this.allianceTab);
            ...
            PendingPopulate: function() {
                ...
                if (details) {
                    ...
                    for (var i = 0; i < pendingInvites.length; i++) {
                        ...
                        Dom.addClass(nLi, 'embassyMessage');
                        nLi.innerHTML =
                            '<label>Reason:</label><input type="text" class="embassyPendingActionMessage" />';
                        nUl.appendChild(nLi);
            ...
            PendingWithdraw: function() {
                if (
                    confirm(
                        [
                            'Are you sure you want to withdraw the invite from ',
                            this.Invite.name,
                        ].join('')
                    )
                ) {
                        ...
                        },
                        {
                            success: function(o) {
                                YAHOO.log(
                                    o,
                                    'info',
                                    'Embassy.withdraw_invite.success'
                                );
                                this.Self.rpcSuccess(o);
            ...
            SendInvite: function() {
                var inviteeId =
                    (this.embassySendTo._oTblSingleSelection &&
                        this.embassySendTo._oTblSingleSelection.Object.id) ||
                    null;
            ...
            MembersPopulate: function() {
                ...
                if (details && this.alliance) {
                    ...
                    for (var i = 0; i < members.length; i++) {
                        ...
                        if (
                            this.isLeader &&
                            this.alliance.leader_id != obj.empire_id
                        ) {
                            nLi = li.cloneNode(false);
                            Dom.addClass(nLi, 'embassyAction');
                            ...
                            Dom.addClass(nLi, 'embassyMessage');
                            nLi.innerHTML =
                                '<label>Reason:</label><input type="text" class="embassyMembersMessage" />';
                            nUl.appendChild(nLi);
                        }
            ...
            MembersPromote: function() {
                if (
                    confirm(
                        [
                            'Are you sure you want to transfer alliance control to ',
                            this.Member.name,
                            '?',
                        ].join('')
                    )
                        ...
                        },
                        {
                            success: function(o) {
                                YAHOO.log(
                                    o,
                                    'info',
                                    'Embassy.assign_alliance_leader.success'
                                );
                                this.Self.rpcSuccess(o);
                                this.Self.alliance = o.result.alliance;
                                this.Self.isLeader =
            ...
            PropLineDetails: function(prop, sec) {
                if (prop.status == 'Passed' || prop.status == 'Failed') {
                    return [
                        ...
                        prop.name,
                        '</label></div>',
                        '    <div class="yui-u">Proposed by <a class="profile_link" href="#',
                        prop.proposed_by.id,
                        '">',
                        prop.proposed_by.name,
                        '</a></div>',
                        '    <div class="yui-u"><label>',
                        prop.status,
                        '</label></div>',
                        '</div>',
                        '<div class="yui-gc">',
                        '    <div class="yui-u first"><div class="propDesc">',
                        this.formatBody(prop.description),
                        '</div></div>',
                        '    <div class="yui-u"><div class="propMyVote">',
                        this.PropVoteDetails(prop),
                        '</div></div>',
                        '</div>',
            ...
            formatBody: function(body) {
                ...
                body = body.replace(
                    /\{Starmap\s+(-?\d+)\s+(-?\d+)\s+([^\}]+)\}/gi,
                    '<a class="starmap_link" href="#$1x$2">$3</a>'
                );
                body = body.replace(
                    /\{Planet\s+(-?\d+)\s+([^\}]+)\}/gi,
                    '<a class="planet_link" href="#$1">$2</a>'
                );
                body = body.replace(
                    /\{Alliance\s+(-?\d+)\s+([^\}]+)\}/gi,
                    '<a class="alliance_link" href="#$1">$2</a>'
                );
                body = body.replace(
                    /\{VoteYes\s(-*\d+)\s(-*\d+)\s(-*\d+)\}/gi,
                    '<a class="voteyes_link" href="#$1&$2&$3">Yes!</a>'
                );
                body = body.replace(
                    /\{VoteNo\s(-*\d+)\s(-*\d+)\s(-*\d+)\}/gi,
                    '<a class="voteno_link" href="#$1&$2&$3">No!</a>'
                );
