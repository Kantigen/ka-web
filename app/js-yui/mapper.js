    ...
    (function() {
        ...
        Mapper.VisibleArea.prototype = {
            move: function(mx, my) {
                ...
                if (mb) {
                    ...
                    if (mx < 0 && cb.x1 < mb.x1Left) {
                        ...
                        //if moving right
                        mx =
                            (mb.x2Right + 1) * tileSize -
                            (this.left + maxWidth - extraSpaceRight);
                    }
                    //y axis
            ...
            centerLoc: function() {
                var tileSize = this._map.tileSizeInPx;
                return [
                    Math.floor(this.centerX / tileSize),
        ...
        var Tile = function(x, y, z, ox, oy, layer) {
            ...
            if (this.url) {
                s.background =
                    'transparent url(' + this.url + ') no-repeat scroll center';
            }
        ...
        Tile.prototype = {
            ...
            appendToDom: function() {
                if (
                    this.domElement &&
                    !this.blank &&
                    !Dom.isAncestor(this.layer.tileContainer, this.domElement)
                ) {
        ...
        Mapper.StarTile = function(x, y, z, ox, oy, layer) {
            Mapper.StarTile.superclass.constructor.call(
                this,
                x,
                y,
                z,
        ...
        Lang.extend(Mapper.StarTile, Tile, {
            init: function() {
                this.domElement.title = this.data
                    ? [this.data.name, ' (', this.x, ',', this.y, ')'].join('')
                    : 'Uncharted Space';
            ...
            _createImage: function() {
                if (!this.imageHolder) {
                    var image = this.domElement.appendChild(
                ...
                if (this.data.orbit) {
                    if (this.map.hidePlanets) {
                        ...
                        var pSize =
                            (100 - Math.abs(this.data.size - 100)) /
                                (100 / this.tileSizeInPx) +
                            15;
                        this.imageHolder.innerHTML = [
                            ...
                            pSize,
                            'px;margin-top:',
                            Math.floor((this.tileSizeInPx - pSize) / 2),
                            'px;" />',
                        ].join('');

                        if (this.data.body_has_fissure) {
                            this.fissureHolder.innerHTML = [
                                '<img src="',
                                Lib.AssetUrl,
                                'star_map/fissure_icon.png" class="planet" style="width:',
                                pSize,
                                'px;height:',
                                pSize,
                                'px;margin-top:',
                    ...
                    if (this.data.station) {
                        ...
                        this.logoHolder.innerHTML = [
                            '<img src="',
                            Lib.AssetUrl,
                            'alliances/',
                            station.alliance.image,
                            '.png" class="star" style="width:',
                            pSize,
                            'px;height:',
                            pSize,
                            'px;opacity:',
                        ...
                        if (this.map.hidePlanets) {
                            ...
                            var pSize =
                                (100 - Math.abs(this.data.size - 100)) /
                                    (100 / this.tileSizeInPx) +
                                15;
                            this.alignHolder.innerHTML = [
                                '<img src="',
                                Lib.AssetUrl,
                                'star_map/',
                                this.data.empire.alignment,
                                '.png" class="planet" style="width:',
                                pSize,
                                'px;height:',
                                pSize,
                                'px;margin-top:',
            ...
            _buildLogoHolder: function() {
                if (!this.logoHolder) {
                    var logo = this.domElement.appendChild(
                        document.createElement('div')
                    );
            ...
            _buildFissureHolder: function() {
                if (!this.fissureHolder) {
                    var fissure = this.domElement.appendChild(
                        document.createElement('div')
                    );
            ...
            _buildAlignmentHolder: function() {
                if (!this.alignHolder) {
                    var align = this.domElement.appendChild(
                        document.createElement('div')
                    );
        ...
        Mapper.PlanetTile = function(x, y, z, ox, oy, layer) {
            Mapper.PlanetTile.superclass.constructor.call(
                this,
                x,
                y,
                z,
        ...
        Lang.extend(Mapper.PlanetTile, Tile, {
            ...
            startTick: function() {
                if (this.data) {
                    ...
                    if (
                        this.data.pending_build &&
                        this.data.pending_build.seconds_remaining > 0
                    ) {
                        this._createBuildCounter();
                        subTick = true;
                    } else if (this.counterBuild) {
                        this.counterBuild.parentNode.removeChild(
                            this.counterBuild
                        );
                        delete this.counterBuild;
                    }
                    ...
                    if (
                        this.data.work &&
                        this.data.work.seconds_remaining > 0
                    ) {
                        this._createWorkCounter();
                        subTick = true;
                    } else if (this.counterWork) {
                        this.counterWork.parentNode.removeChild(
                            this.counterWork
                        );
            ...
            tick: function(e, oArgs) {
                ...
                if (this.data.pending_build) {
                    ...
                    var remainingBuild = Math.round(
                        this.data.pending_build.seconds_remaining
                    );
                    ...
                    this.counterBuild.innerHTML = Lib.formatTime(
                        remainingBuild
                    );
                }
                if (this.data.work) {
                    this.data.work.seconds_remaining -= tickSec;
                    var remainingWork = Math.round(
                        this.data.work.seconds_remaining
                    );
                    if (remainingWork < 0) {
                        remainingWork = 0;
                    }
            ...
            */
            ...
            _createBuildCounter: function() {
                if (!this.counterBuild) {
                    var counterBuild = this.domElement.appendChild(
                        document.createElement('div')
                    );
            ...
            _createWorkCounter: function() {
                if (!this.counterWork) {
                    var counterWork = this.domElement.appendChild(
                        document.createElement('div')
                    );
                ...
                if (this.actionIcon) {
                    if (this.data) {
                        ...
                        this.actionIcon.innerHTML = [
                            '<div class="planetMapTileActionLevel">',
                            this.data.level,
                            '</div>',
                        ].join('');
                    } else {
                        this.actionIcon.innerHTML =
                            '<div class="planetMapTileActionButton"></div>';
                    }
                } else {
                    var div = this.domElement.appendChild(
                        document.createElement('div')
                    );
                    ...
                    Dom.setStyle(
                        div,
                        'height',
                        Math.round(this.tileSizeInPx / 2) + 'px'
                    );
                    if (this.data) {
                        if (Game.GetCookieSettings('showLevels', '0') == '1') {
            ...
            _createEfficiencyBar: function(efficiency) {
                if (!this.cBar) {
                    var bar = this.domElement.appendChild(
                        document.createElement('div')
                    );
                ...
                Dom.setStyle(
                    this.eBar,
                    'width',
                    Math.floor(this.tileSizeInPx * (efficiency / 100)) + 'px'
                );
        ...
        var TileLayer = function(map, visibleArea, TileConstructor) {
            ...
            this.tileContainer = map.movableContainer.appendChild(
                tileContainer
            );
        ...
        TileLayer.prototype = {
            ...
            tileAtPosition: function(x, y) {
                ...
                return [
                    Math.floor(x / tileSizeInPx),
                    Math.floor(y / tileSizeInPx),
                ];
            },
        ...
        Mapper.StarTileLayer = function(map, visibleArea, TileConstructor) {
            Mapper.StarTileLayer.superclass.constructor.call(
                this,
                map,
                visibleArea,
                TileConstructor
                        ...
                        for (var yc = bounds.y2; yc <= bounds.y1; yc++) {
                            ...
                            if (tile) {
                                if (tile.data && tile.data.name) {
                                    if (
                                        planets.hasOwnProperty(tile.data.name)
                                    ) {
                                        if (
                                            ...
                                            delete Lacuna.MapStar._map
                                                .tileCache[tile.x][tile.y]; // Remove the planet from the cache
                                        }
                                        tile.blank = true;
                                    }
                                }
        ...
        Mapper.PlanetTileLayer = function(map, visibleArea, TileConstructor) {
            ...
            Mapper.PlanetTileLayer.superclass.constructor.call(
                this,
                map,
                visibleArea,
                TileConstructor
        ...
        Map.prototype = {
            ...
            moveByTiles: function(x, y) {
                var absX = Math.abs(x),
                    ...
                    durMS =
                        absX > absY
                            ? this.tileSizeInPx * absX
                            : this.tileSizeInPx * absY, //get the longest duration if we're moving both x and y
            ...
            redraw: function() {
                ...
                this.tileLayer = new this.TileLayer(
                    this,
                    Mapper.util.clone(this.visibleArea),
                    this.Tile
                );
                //pass through
                this.tileLayer.subscribe(
                    'onReloadTile',
                    function(tile) {
            ...
            refresh: function(isZoom) {
                ...
                var vac = isZoom
                    ? this.visibleArea.centerLoc()
                    : this.visibleArea.centerLocPx();
                //set tile size
            ...
            setCenterTo: function(locX, locY) {
                if (
                    ...
                    var otherWidth = this.visibleArea.centerX,
                        ...
                        ox =
                            locX * this.tileSizeInPx +
                            this.tileSizeInPx / 2 -
                            otherWidth,
                        oy =
                            locY * -1 * this.tileSizeInPx +
                            this.tileSizeInPx / 2 -
                            otherHeight;
        ...
        Mapper.StarMap = function(divId, options) {
            ...
            Dom.setStyle(
                this.mapDiv,
                'background-image',
                'url("' + Lib.AssetUrl + 'star_system/field.png")'
            );
        };
        Lang.extend(Mapper.StarMap, Map, {
            ...
            getTileData: function(callback, x1, x2, y1, y2) {
                ...
                if (xDiff * yDiff > MAX_STAR_AREA) {
                    //if out of bounds split and try again
                    var half;
                    if (xDiff > 0) {
                        //make sure x diff isn't zero so we can split it in half, other wise use Y axis
        ...
        Lang.extend(Mapper.PlanetMap, Map, {
            ...
            addTileData: function(oTiles, filterOutDeleted) {
                ...
                for (var tKey in oTiles) {
                    if (oTiles.hasOwnProperty(tKey)) {
                        ...
                        tile.id = tKey;
                        if (
                            tile.url == '/planetarycommand' ||
                            tile.url == '/stationcommand'
                        ) {
                            this.command = tile;
                            this.command.x *= 1;
                            this.command.y *= 1;
            ...
            addSingleTileData: function(oBuilding) {
                if (
                    oBuilding.url == '/planetarycommand' ||
                    oBuilding.url == '/stationcommand'
                ) {
            ...
            refreshTile: function(building) {
                if (this.tileLayer) {
                    ...
                    var tile = this.tileLayer.findTile(
                        building.x,
                        building.y,
                        this.zoom
                    );
        ...
        Mapper.TraditionalController = function(map) {
            ...
            var moveKeyListener = new KL(
                document,
                {
                    keys: [KL.KEY.UP, KL.KEY.DOWN, KL.KEY.LEFT, KL.KEY.RIGHT],
                },
                { fn: this.moveKey, scope: this, correctScope: true },
                KL.KEYDOWN
            );
            var moveKeyUpListener = new KL(
                document,
                {
                    keys: [KL.KEY.UP, KL.KEY.DOWN, KL.KEY.LEFT, KL.KEY.RIGHT],
                },
                { fn: this.moveKeyUp, scope: this, correctScope: true },
                KL.KEYUP
            );
            ...
            if (map.maxZoom - map.minZoom != 0) {
                ...
                Event.on(
                    Sel.query('.mapiator_zoom_in', zoomEl, true),
                    'click',
                    map.zoomIn,
                    map,
                    true
                ...
                Event.on(
                    Sel.query('.mapiator_zoom_out', zoomEl, true),
                    'click',
                    map.zoomOut,
                    map,
                    true
                );
                var sliderId = Dom.generateId(
                    Sel.query('.mapiator_zoom_slider', zoomEl, true),
                    'mapiator_zoom_slider'
                );
                var thumbId = Dom.generateId(
                    Sel.query('.mapiator_zoom_slider_thumb', zoomEl, true),
                    'mapiator_zoom_slider_thumb'
                );

                var zoomSize = 140;
                var zoomScale = Math.floor(
                    zoomSize / (map.maxZoom - map.minZoom)
                );
