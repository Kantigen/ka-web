        ...
        MapStar.prototype = {
            ...
            _buildPlanetDetailsPanel: _.once(function() {
                ...
                this.planetDetails.renderEvent.subscribe(
                    function() {
                        ...
                        var tv = (this.planetDetails.tabView = new YAHOO.widget.TabView(
                            'planetDetailTabs'
                        ));
                        ...
                        for (var tabId in spyTabs) {
                            if (spyTabs.hasOwnProperty(tabId)) {
                                ...
                                tabEl.elSpyShipsPane = Sel.query(
                                    '.planetDetailSelectSpyShip',
                                    tabEl,
                                    true
                                );
                                tabEl.elSendButton = Sel.query(
                                    '.planetDetailSelectSpies button',
                                    tabEl,
                                    true
                                );
                                tabEl.elSpiesList = Sel.query(
                                    '.planetDetailSpiesList',
                                    tabEl,
                                    true
                                );
                                tabEl.elSpyShipsList = Sel.query(
                                    '.planetDetailSpyShipList',
                                    tabEl,
                                    true
                                );
                                tabEl.elMessage = Sel.query(
                                    '.planetDetailSpiesMessage',
                                    tabEl,
                                    true
                                );
            ...
            NotFleetIsolationist: function(ships) {
                if (Game.EmpireData.is_isolationist == '1') {
                    ...
                    for (var n = 0; n < ships.length; n++) {
                        if (
                            ships[n].type == 'colony_ship' ||
                            ships[n].type == 'short_range_colony_ship'
                        ) {
                            hasIsoShip = true;
                            break;
                        }
            ...
            populateSpies: function(tab) {
                ...
                if (maxSpies == 0) {
                    ...
                    tab.elMessage.innerHTML =
                        'Select up to ' + maxSpies + ' spies to ' + verb + ':';
            ...
            MoveSpies: function(e, tab) {
                ...
                if (spies.length > tab.maxSpies) {
                    alert(
                        "You don't have any ships large enough to " +
                            verb +
                            ' ' +
                            spies.length +
                            ' spies.'
