    ...
    (function() {
        ...
        Messaging.prototype = {
            _buildPanel: _.once(function() {
                ...
                this.messagingPanel.renderEvent.subscribe(
                    function() {
                        //tabs
                        this.create = Dom.get('messagingCreate');
                        this.inbox = Dom.get('messagingInbox');
                        this.inboxTag = Dom.get('inboxTag');
                        this.tag = this.inboxTag.options[
                            this.inboxTag.selectedIndex
                        ].value;
                        Event.on(
                            'inboxTag',
                            'change',
                        ...
                        this.archiveButton = Dom.get(
                            'messagingArchiveSelected'
                        );
                        Event.on(
                            this.archiveButton,
                            'click',
                ...
                this.messagingPanel.hideEvent.subscribe(
                    function() {
                    ...
                    if (map) {
                        Dom.setStyle(
                            ...
                            [
                                ...
                                Lib.AssetUrl,
                                'planet_side/',
                                map.surface || map.surface_image,
                                '.jpg") repeat scroll 0 0 black',
                            ].join('')
                        );
            ...
            _createToSelect: function() {
                ...
                oTextboxList.doBeforeLoadData = function(
                    sQuery,
                    oResponse,
                    oPayload
                ) {
                ...
                switch (el.id) {
                    ...
                    case this.announce.id:
                        ...
                        Dom.get('messagingAnnounceFrame').src =
                            '/announcement?session_id=' + Game.GetSession();
                        break;
                    default:
            ...
            loadCreate: function(isAll) {
                ...
                if (this.viewingMessage) {
                    if (isAll) {
                        ...
                        for (
                            var i = 0;
                            i < this.viewingMessage.recipients.length;
                            i++
                        ) {
                    ...
                    this.createSubject.value =
                        (this.viewingMessage.forwarding ? 'Fwd: ' : 'Re: ') +
                        this.viewingMessage.subject;
                    this.createText.value =
                        '\n\n--------------\nOn ' +
                        Lib.formatServerDate(this.viewingMessage.date) +
                        ' ' +
                        this.viewingMessage.from +
                        ' wrote:\n' +
            ...
            processMessages: function(results, is) {
                ...
                if (messages.length == 0) {
                    ...
                    for (var i = 0; i < messages.length; i++) {
                        ...
                        nLi.innerHTML = [
                            ' <div class="messageSelect"><img width="26" height="26" src="',
                            Lib.AssetUrl,
                            'ui/mail-',
                            img,
                            '.png" /><br />',
                            !isTab.sent ? '    <input type="checkbox" />' : '',
                            '</div>',
                            '    <div class="messageContainer">',
                            '        <div class="messageDate">',
                            Lib.formatServerDate(msg.date),
                            '</div>',
            ...
            */
            ...
            sendMessage: function() {
                ...
                if (to.length == 0) {
                    this.createResponse.innerHTML =
                        'Must send a message To someone.';
                } else {
                    var InboxServ = Game.Services.Inbox,
                        data = {
                            session_id: Game.GetSession(''),
            ...
            archiveProcess: function(results) {
                Dom.batch(
                    Sel.query('li.message', this.list),
                    function(el) {
