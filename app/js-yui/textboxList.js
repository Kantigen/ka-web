...
//only load this once
if (
    typeof YAHOO.lacuna.TextboxList == 'undefined' ||
    !YAHOO.lacuna.TextboxList
) {
    ...
    (function() {
        ...
        var TBL = function(elInput, oDataSource, oConfigs) {
            ...
            oConfigs.formatResultLabelKey =
                oConfigs.formatResultLabelKey ||
                oDataSource.responseSchema.fields[0].key ||
                oDataSource.responseSchema.fields[0];
            oConfigs.formatResultColumnKeys = oConfigs.formatResultColumnKeys || [
                oDataSource.responseSchema.fields[0].key ||
                    oDataSource.responseSchema.fields[0],
            ]; //used to show correct columns in drop down
            ...
            if (oConfigs.useIndicator) {
                ...
                tblIndicator.innerHTML = [
                    '<img src="',
                    Lib.AssetUrl,
                    'ui/indicator.gif" alt="Loading..." />',
                ].join('');
            ...
            TBL.superclass.constructor.call(
                this,
                tblInput,
                tblAC,
                oDataSource,
            ...
            this._elTblListInputLine = this.multiSelect
                ? tblListInputLine
                : undefined;
            ...
            if (this.useIndicator) {
                ...
                this.dataReturnEvent.subscribe(function(
                    oSelf,
                    sQuery,
                    oRequest
                ) {
        ...
        Lang.extend(TBL, YAHOO.widget.AutoComplete, {
            ...
            handleResponseSingle: function(sQuery, oResponse, oPayload) {
                if (this instanceof YAHOO.widget.AutoComplete && this._sName) {
                    ...
                    //get all list items
                    var elListItem,
                        allListItemEls = this._elList.childNodes,
                        queue =
                            oPayload && oPayload.queue ? oPayload.queue : null;
                    if (queue) {
                        for (var i = 0; i < allListItemEls.length; i++) {
                            elListItem = allListItemEls[i];
                            if (
                                elListItem._oResultData &&
                                queue.indexOf(
                                    String(elListItem._oResultData.Id)
                                ) >= 0
                            ) {
            ...
            _populateList: function(sQuery, oResponse, oPayload) {
                ...
                contentStyle.height = (!isOpera) ? null : "";*/
                    ...
                    if (nItemsToShow > 0) {
                        ...
                        if (
                            !this._elList ||
                            this._elList.childNodes.length < nItemsToShow
                        ) {
                            this._initListEl();
                        }
                        ...
                        for (var i = nItemsToShow - 1; i >= 0; i--) {
                            ...
                            if (this.resultTypeList) {
                                ...
                                var aResult = [];
                                // Match key is first
                                aResult[0] = YAHOO.lang.isString(oResult)
                                    ? oResult
                                    : oResult[sMatchKey] || oResult[this.key];
                                // Add additional data to the result array
                                var fields = this.dataSource.responseSchema
                                    .fields;
                                if (
                                    YAHOO.lang.isArray(fields) &&
                                    fields.length > 1
                                ) {
                                    for (
                                        var k = 1, len = fields.length;
                                        k < len;
                                        k++
                                    ) {
                                        aResult[aResult.length] =
                                            oResult[fields[k].key || fields[k]];
                                    }
                                }
                            ...
                            elListItem._sResultMatch = YAHOO.lang.isString(
                                oResult
                            )
                                ? oResult
                                : YAHOO.lang.isArray(oResult)
                                ? oResult[0]
                                : oResult[sMatchKey] || '';
                            elListItem._oResultData = oResult; // Additional data
                            elListItem.innerHTML = this.formatResult(
                                oResult,
                                sCurQuery,
                        ...
                        this.containerPopulateEvent.fire(
                            this,
                            sQuery,
                            allResults
                        );
                        ...
                        if (!this._singleRequest) {
                            ...
                            ok = this.doBeforeExpandContainer(
                                this._elTextbox,
                                this._elContainer,
                                sQuery,
                                allResults
            ...
            _selectItem: function(elListItem) {
                if (
                    ...
                    if (this.multiSelect) {
                        ...
                        var oData = this._createDataObject(
                            elListItem._oResultData
                        );
            ...
            _updateDirty: function() {
                ...
                if (!this._singleRequest) {
                    ...
                    if (this.multiSelect) {
                        ...
                        isDirty =
                            this._sOrigSingleSelection !=
                            this._oTblSingleSelection.Value;
                    }
