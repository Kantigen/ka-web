    ...
    (function() {
        ...
        MapPlanet.prototype = {
            _buildDetailsPanel: _.once(function() {
                ...
                this.buildingDetails.addQueue = function(
                    seconds,
                    queueFn,
                    elm,
                    sc
            ...
            _buildBuilderPanel: _.once(function() {
                ...
                this.buildingBuilder.clickBuildMenu = function(
                    e,
                    matchedEl,
                    container
                ) {
                ...
                this.buildingBuilder.viewBuildings = function(mainTag, subTag) {
                    ...
                    if (subTag) {
                        ...
                        this.filterTrail.innerHTML =
                            ' &gt; <span class="buildMenuLink">' +
                            displayTag +
                            '</span>';
                    }
                    var filterTag = subTag ? subTag : mainTag;
                    if (
                        !this.buildable[mainTag] &&
                        !this.buildable[filterTag]
                    ) {
                ...
                this.buildingBuilder.updateDisplay = function() {
                    ...
                    var isQueueFull =
                        this.queue_status.max == this.queue_status.current;
                    ...
                    for (var i = 0; i < names.length; i++) {
                        ...
                        nLi.innerHTML = [
                            ...
                            '</td>',
                            '                <td class=',
                            costs.water > planet.water_stored
                                ? 'low-resource'
                                : '',
                            '>',
                            costs.water,
                            '</td>',
                            '                <td class=',
                            costs.energy > planet.energy_stored
                                ? 'low-resource'
                                : '',
                            '>',
                            costs.energy,
                            ...
                            '            <tr><th>Prod:</th>',
                            '                <td class=',
                            -1 * prod.food_hour > planet.food_hour
                                ? 'low-resource'
                                : '',
                            '>',
                            prod.food_hour,
                            '/hr</td>',
                            '                <td class=',
                            -1 * prod.ore_hour > planet.ore_hour
                                ? 'low-resource'
                                : '',
                            '>',
                            prod.ore_hour,
                            '/hr</td>',
                            '                <td class=',
                            -1 * prod.water_hour > planet.water_hour
                                ? 'low-resource'
                                : '',
                            '>',
                            prod.water_hour,
                            '/hr</td>',
                            '                <td class=',
                            -1 * prod.energy_hour > planet.energy_hour
                                ? 'low-resource'
                                : '',
                            '>',
                            prod.energy_hour,
            ...
            Mapper: function(oArgs) {
                ...
                for (var key in this.buildings) {
                    if (this.buildings.hasOwnProperty(key)) {
                        this.buildings[key] = this.CleanBuilding(
                            this.buildings[key]
                        );
                    }
                }
                ...
                if (!this._gridCreated) {
                    ...
                    map.setPlotsAvailable(
                        oArgs.status.body.plots_available * 1
                    );
            ...
            BuilderView: function(tile) {
                ...
                Dom.setStyle(
                    Dom.get('builderList').parentNode,
                    'height',
                    Ht + 'px'
                );
            },
            BuilderGet: function(data) {
                require('js/actions/menu/loader').show();
                this.currentBuildConnection = Game.Services.Body.get_buildable(
                    data,
                    {
                        success: function(o) {
            ...
            ViewData: function(id, url, callback, x, y) {
                ...
                return BuildingServ.view(data, {
                    success: function(o) {
                        ...
                        newB.y = y;
                        newB.updated =
                            !this.buildings[newB.id] ||
                            newB.level != this.buildings[newB.id].level;
            ...
            BuildingFactory: function(result) {
                var classConstructor =
                        FactoryMap[result.building.url] ||
                        Lacuna.buildings.Building,
                    classObj = new classConstructor(result, this.locationId);
                if (classObj) {
                    classObj.subscribe(
                        'onMapRpc',
                        this._fireRpcSuccess,
                        this,
                        true
                    );
                    classObj.subscribe(
                    ...
                    classObj.subscribe(
                        'onReloadTabs',
                        this._fireReloadTabs,
                        this,
                        true
                    );
                    classObj.subscribe(
                        'onUpdateTile',
                        this._fireUpdateTile,
                        this,
                        true
                    );
                    classObj.subscribe(
            ...
            DetailsProcess: function(oResults, url, x, y) {
                var building = oResults.building,
                    ...
                    currBuildingId = this.currentBuilding
                        ? this.currentBuilding.building.id
                        : undefined;
                if (
                    panel.isVisible() &&
                    currBuildingId != oResults.building.id
                ) {
                    building.url = url;
                    building.x = x;
                    building.y = y;
                                ...
                                function(remaining, elm) {
                                    ...
                                    if (rf <= 0) {
                                        ...
                                        YAHOO.log(
                                            'Complete',
                                            'info',
                                            'buildingDetails.showEvent.BuildTimeRemaining'
                                        );
                                        this.DetailsView({
                                            data: {
                                                id: building.id,
